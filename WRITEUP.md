...
Image size in general will always be different depending on the specification of the cameras or the inputs that we use. We need to preprocess this images to account for different posible ordering of color channels, reshaping the inputs to match the expected model input size,  and sometimes removing noise or applying some transformations on the models inputs like pixels value normalization or additional techniques like edge detection (if applicable). Usually its necessary to reshape the original image into a smaller resolution which requires that some 'aggregation' needs to be performed internally to the reduce the number of different image pixels. Nevertheless , it could be possible that is necessary to reshape the image into a bigger resolution, in which case we would need to apply any interpolation technique (like when applying a digital zoom in our cellphones).

It is also key to have into consideration  the focal length of the camera that we are going to use in our application, because if the target objects are being captured out of focus our system performance could decrease considerably. The situation is similar with other environmental factors like lightning. Why are this factors so important? Always remember that a computer doesn't perceive images in the same way that we humans do. For a computer a image is simply a bunch of matrixes with represent each of the differnt pixel colors, so even if we are able to distinguish objects in several lightning conditions its very likely that the underlying image matrices will be very different even if the real objects are the same. 
